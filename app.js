...
/* Sanfter Content-Wechsel + Weiche Kartenhöhen-Animation */
function updateCardContent(renderFn) {
  ...
  setTimeout(() => {
    // Neuen Inhalt in card-inner rendern
    card.innerHTML = `<div class="card-inner" id="cardInner"></div>`;
    ...
    // Auf Start-Höhe zurücksetzen und für Animation zur Zielhöhe zwingen
    card.style.height = `${startHeight}px`;
    card.offsetHeight; // Reflow erzwingen

    card.style.height = `${targetHeight}px`;
    card.classList.remove('is-transitioning');

    // Nach Abschluss der Animation feste Höhe aufheben für Responsive Design
    setTimeout(() => {
      card.style.height = '';
    }, 260);
  }, 180);
...
function renderFetchingRate() {
  updateCardContent((container) => {
    container.innerHTML = `<div class="question">Getting exchange rate…</div>`;
  });
}
